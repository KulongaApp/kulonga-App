-- KULONGA — SCHEMA CORRIGIDO (v2) — execução
-- Extensões
...
CREATE POLICY "professores_ver_propria_linha" ON professores FOR SELECT USING (user_id = auth.uid());
CREATE POLICY "encarregados_ver_propria_linha" ON encarregados FOR SELECT USING (user_id = auth.uid());

CREATE POLICY "escolas_staff" ON escolas FOR ALL USING (id = ANY(escolas_do_staff()));
CREATE POLICY "professores_staff" ON professores FOR ALL USING (user_id = auth.uid() OR escola_id = ANY(escolas_do_staff()));
CREATE POLICY "alunos_staff" ON alunos FOR ALL USING (escola_id = ANY(escolas_do_staff()) OR id = ANY(alunos_do_encarregado()));
CREATE POLICY "turmas_staff" ON turmas FOR ALL USING (
  escola_id = ANY(escolas_do_staff())
  OR id IN (SELECT turma_id FROM disciplina_professor WHERE professor_id IN (SELECT id FROM professores WHERE user_id = auth.uid()))
);
CREATE POLICY "disciplinas_staff" ON disciplinas FOR ALL USING (escola_id = ANY(escolas_do_staff()));
CREATE POLICY "turma_alunos_staff" ON turma_alunos FOR ALL USING (
  turma_id IN (SELECT id FROM turmas WHERE escola_id = ANY(escolas_do_staff()))
  OR aluno_id = ANY(alunos_do_encarregado())
);
CREATE POLICY "turma_disciplinas_staff" ON turma_disciplinas FOR ALL USING (
  turma_id IN (SELECT id FROM turmas WHERE escola_id = ANY(escolas_do_staff()))
);
CREATE POLICY "disciplina_professor_staff" ON disciplina_professor FOR ALL USING (
  disciplina_id IN (SELECT id FROM disciplinas WHERE escola_id = ANY(escolas_do_staff()))
);
CREATE POLICY "notas_staff" ON notas FOR SELECT USING (
  turma_id IN (SELECT id FROM turmas WHERE escola_id = ANY(escolas_do_staff()))
  OR aluno_id = ANY(alunos_do_encarregado())
);
CREATE POLICY "notas_inserir" ON notas FOR INSERT WITH CHECK (
  autor_id = auth.uid()
  AND turma_id IN (SELECT id FROM turmas WHERE escola_id = ANY(escolas_do_staff()))
);
CREATE POLICY "notas_actualizar" ON notas FOR UPDATE USING (autor_id = auth.uid());
CREATE POLICY "encarregados_staff" ON encarregados FOR ALL USING (
  user_id = auth.uid()
  OR id IN (SELECT encarregado_id FROM encarregado_aluno WHERE aluno_id = ANY(alunos_do_encarregado()))
);
CREATE POLICY "encarregado_aluno_staff" ON encarregado_aluno FOR ALL USING (
  encarregado_id IN (SELECT id FROM encarregados WHERE user_id = auth.uid())
  OR aluno_id = ANY(alunos_do_encarregado())
);
CREATE POLICY "tokens_validar" ON tokens_acesso FOR SELECT USING (ativo = true);
CREATE POLICY "tokens_gerar" ON tokens_acesso FOR INSERT WITH CHECK (
  gerado_por = auth.uid()
  AND aluno_id IN (SELECT id FROM alunos WHERE escola_id = ANY(escolas_do_staff()))
);
CREATE POLICY "notas_pendentes_ver" ON notas_pendentes FOR SELECT USING (autor_id = auth.uid());
CREATE POLICY "notas_pendentes_criar" ON notas_pendentes FOR INSERT WITH CHECK (autor_id = auth.uid());
CREATE POLICY "notas_pendentes_actualizar" ON notas_pendentes FOR UPDATE USING (autor_id = auth.uid());
...
CREATE POLICY "upload_foto_aluno" ON storage.objects
  FOR INSERT WITH CHECK (bucket_id = 'fotos-alunos' AND auth.role() = 'authenticated');
